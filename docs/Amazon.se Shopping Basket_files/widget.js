    ...
    P.when('A').execute(function(A) {
        ...
        var Modal = function() {
                ...
                if (isMobile) {
                   ...
                   //create bottom-sheet
                    var config = {
                        ...
                        height: height(),
                        closeType: 'icon',
                        sheetLabel: '#headline-fallback-placeholder'
                    };
            ...
            this.show = function(repaint) {
                ...
                modalInstance.show();
                // we want to unhide the bottomsheet close button if hidden due to payment widget CSS
                // see https://sim.amazon.com/issues/APXClient-4338
                if (isMobile) {
                    const bottomsheetElem = document.querySelector('[aria-labelledby="headline-fallback-placeholder"]');
                    bottomsheetElem?.style.setProperty('overflow-y', 'visible', 'important');
                }
    ...
    P.when('A', 'wlp-injectable-modal', 'usp-wlp-utility').register(
        ...
        function(A, modal, utility) {
            ...
            var _loadPopup = function(input, preFetchId) {
                function extractHeadingText(result, headingID) {
                    const temp = document.createElement('div');
                    temp.innerHTML = result;
                    const headingElem = temp.querySelector(`#${headingID}`);
                    if (!headingElem) {
                        return 'Prime Signup Dialog';
                    }
                    ...
                    utility.PreFetchService().get(preFetchId, function(success, result, togglePreFetch) {
                        ...
                        if (success) {
                            ...
                            if (!result || result.indexOf('USPWLPPipelineLoadedSuccessfully') == -1) {
                                ...
                                var label = extractHeadingText(result, 'headline-fallback-placeholder')
                                // Show close button for success modal (for accessibility)
                                modal.update({
                                    'closeButton': true,
                                    'popoverLabel': label
                                });
